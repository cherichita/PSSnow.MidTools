...
task SnowMutualAuthAddRoot SnowMidInitializeTools, {
    $Script:RootCN = "az-mid-ca-$($env:SN_MID_ENVIRONMENT_NAME)"
    $Script:MutualAuthAvailable = Get-SNOWMidMutualAuthState -RootCN $Script:RootCN
    $Script:CoreCert = Set-SNOWMidMutualAuthRoot -RootCN $Script:RootCN
}

task EnableUserMutualAuth SnowMutualAuthAddRoot, {
    $RootCA = $Script:CoreCert.RootCA
    $ctx = Resolve-SNOWMIDBuildContext -ErrorAction Stop
    $MyUser = Get-SNOWCurrentUser -ErrorAction Stop
    $LeafCN = "user-$($MyUser.sys_id)-${env:SN_MID_ENVIRONMENT_NAME}"
    if ($MyUser) {
